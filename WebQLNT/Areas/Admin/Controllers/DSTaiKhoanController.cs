		...
		public ActionResult Index()
        {
			...
			var userDetail = from nd in db.NguoiDung
							 ...
							 select new NguoiDungViewModel
							 {
								 ...
								 SoLuongNhaTro = grouped.Count(nt => nt.MaNT != null) // Chỉ đếm nếu MaNT không phải là null
							 };
			...
			using (var db = new QLNTEntities())
			{
				using (var transaction = db.Database.BeginTransaction())
				{
					...
					catch (Exception ex)
					{
						...
						return Json(new { success = false, message = "Đã có lỗi xảy ra trong quá trình xóa người dùng" });
					}
				}
			}
		}
